function drawCanvas(data, callback, callbackError, canvasEl) {
  ...
  if (canvasEl && canvasEl.nodeType === 1) {
    canvas = canvasEl;
  }
...
function formatKeys(data, decode) {
  const resKeys = {
    width: 'w',
    height: 'h',
    output: 'o',
  ...
  function getResKey(key) {
    if (!decode) {
      return resKeys[key] || key;
    }
    const tmp = {};
    for (const key in resKeys) {
      tmp[resKeys[key]] = key;
    }
    return tmp[key] || key;
  }
  ...
  for (const key in data) {
    ...
    if (Array.isArray(item)) {
      res[getResKey(key)] = item.map(ii => {
        return formatKeys(ii, decode);
      });
    } else {
      res[getResKey(key)] = item;
    }
  }
  return res;
}

drawCanvas.encodeKeys = function (data) {
  return formatKeys(data);
};

drawCanvas.decodeKeys = function (data) {
  return formatKeys(data, true);
};
